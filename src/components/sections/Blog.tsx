...
const blogPosts = [
  ...
  },
  {
    ...
    description:
      "Tips and tricks to make your website load faster and deliver a smooth user experience.",
    blogURL:
      "https://www.linkedin.com/pulse/why-modern-web-experience-more-empathy-than-code-jabez-samson-52rfc",
  },
  {
...
export function BlogSection() {
  ...
  const visiblePosts = blogPosts.slice(current, current + 3);

  const handlePrev = () => {
    setCurrent((prev) => Math.max(prev - 1, 0));
  };
  const handleNext = () => {
    setCurrent((prev) => (prev + 3 < blogPosts.length ? prev + 1 : prev));
  };
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 md:px-8 relative z-10">
        ...
        <div className="relative w-full flex justify-center">
          <button
            onClick={handlePrev}
            className="absolute left-6 top-1/2 -translate-y-1/2 z-20 bg-gradient-to-br from-primary to-accent text-white rounded-full p-3 shadow-lg hover:scale-110 transition-all duration-200 cursor-pointer"
            aria-label="Previous"
            disabled={current === 0}
          ...
          <div className="w-[80%] grid grid-cols-1 md:grid-cols-3 gap-8">
            {visiblePosts.map((post) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="group bg-card/60 border border-primary/20 rounded-2xl shadow-lg overflow-hidden flex flex-col items-center hover:border-primary/40 transition-all duration-300 relative"
              >
                ...
                <div className="p-6 w-full flex-1 flex flex-col">
                  <h3 className="font-bold text-lg md:text-xl mb-2 bg-gradient-to-r from-primary via-accent to-purple-500 bg-clip-text text-transparent">
                    {post.title}
                  </h3>
          ...
          <button
            onClick={handleNext}
            className="absolute right-6 top-1/2 -translate-y-1/2 z-20 bg-gradient-to-br from-primary to-accent text-white rounded-full p-3 shadow-lg hover:scale-110 transition-all duration-200 cursor-pointer"
            aria-label="Next"
            disabled={current + 3 >= blogPosts.length}
            style={{ opacity: current + 3 >= blogPosts.length ? 0.5 : 1 }}
          >
            <svg
              width="24"
              height="24"
